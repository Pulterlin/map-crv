import {places,docks,allPlaces,contacts,searchPlaces} from './data.js?v=floor-90';
...
function select(id,trigger){
  ...
  $('detail-media').hidden=!p.photo;$('photo-note').textContent=p.photoNote||'现场参考照片·以实际现场为准';if(p.photo){$('detail-photo').src='./photos/'+p.photo;$('detail-photo').alt=p.name+'现场参考照片';}$('dock-picker').replaceChildren();
...
$('detail-photo').onerror=()=>$('detail-media').hidden=true;
$('contact-open').onclick=()=>$('contact-dialog').showModal();$('contact-close').onclick=()=>$('contact-dialog').close();
