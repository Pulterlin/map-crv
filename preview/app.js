import {places,docks,allPlaces,contacts,searchPlaces} from './data.js';
const $=id=>document.getElementById(id);
let map=null,selected=null,filter='all',lastTrigger=null;
const kindNames={building:'园区建筑',dock:'卸货码头',cold:'冷冻库区',service:'园区服务',road:'园区道路'};
function list(){
...
list();
try{const {createMap}=await import('./map.js');map=createMap($('scene'),$('labels'),id=>select(id));$('loading').hidden=true;const initial=new URLSearchParams(location.search).get('place');if(initial)select(initial);}
