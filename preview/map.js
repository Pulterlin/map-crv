...
export function createMap(container,labelLayer,onSelect){
 ...
 function move(target,position,duration=700){transition={start:performance.now(),fromT:controls.target.clone(),toT:target.clone(),fromP:camera.position.clone(),toP:position.clone(),duration:matchMedia('(prefers-reduced-motion: reduce)').matches?0:duration};dirty=true;}
