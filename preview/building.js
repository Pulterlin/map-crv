...
if(!plan){$('floor-title').textContent='暂未提供该栋平面图';$('floor-subtitle').textContent='返回园区全景查看现有信息。';$('floor-board-title').textContent='暂无平面图';}
 ...
 for(const d of ['M370 250V368H486V490H440V696H370V1058H765 M955 1058H1005V1178H1543V250','M370 250H1543','M486 375H650 M742 375H1100 M1100 375H1390','M1100 494H1418 M1100 494V890 M1100 920V940','M1378 660V895 M1378 925V1058','M1390 645H1543','M370 938H505 M545 938H704 M758 938H960 M1000 938H1156 M1210 938H1418 M1468 938H1543','M1426 369H1543 M1426 369V436 M1426 468V493H1543','M765 1058V995H950V1058','M1005 1058H1193 M1253 1058H1332 M1455 1058H1543'])diagram.append(svg('path',{d,class:'plan-wall'}));
 const doors=[
  {x:696,y:375,width:92,sliding:true,name:'前穿堂平移门'},
  {x:875,y:375,sliding:true,name:'前穿堂平移门'},
  {x:1040,y:375,sliding:true,name:'前穿堂平移门'},
  {x:1543,y:350,vertical:true,sliding:true,name:'供应商平移门'},
  {x:1254,y:494,width:56,sliding:true,name:'冷藏区平移门'},
  {x:525,y:938,width:40,sliding:true,name:'后穿堂平移门'},
  {x:731,y:938,width:54,sliding:true,name:'后穿堂平移门'},
  {x:980,y:938,width:40,sliding:true,name:'后穿堂平移门'},
  {x:1183,y:938,width:54,sliding:true,name:'后穿堂平移门'},
  {x:1100,y:905,width:30,vertical:true,sliding:true,name:'冷藏区平移门'},
  {x:1378,y:910,width:30,vertical:true,sliding:true,name:'冷藏区平移门'},
  {x:1543,y:464,vertical:true,name:'员工入口'},
  {x:1543,y:1120,vertical:true,name:'外扩区供应商入口'}
 ];
 ...
 $('floor-zoom-in').onclick=()=>setZoom(zoom*1.25);$('floor-zoom-out').onclick=()=>setZoom(zoom/1.25);$('floor-fit').onclick=()=>setZoom($('floor-viewport').clientWidth/viewW);$('floor-search').addEventListener('input',renderList);for(const button of document.querySelectorAll('[data-floor-filter]'))button.onclick=()=>{filter=button.dataset.floorFilter;for(const other of document.querySelectorAll('[data-floor-filter]')){other.classList.toggle('active',other===button);other.setAttribute('aria-pressed',String(other===button));}renderList();};
 setZoom($('floor-viewport').clientWidth/viewW);renderList();const initial=new URLSearchParams(location.search).get('place');if(initial){const item=all.find(value=>value.id===initial);if(item)choose(item,true);}
}
